...
## Auto-Add New Users To a Team

- Self-hosted AgentLogs deployments can now set `AUTO_ADD_TEAM_ID` to automatically add newly created users to a specific team
- Waitlisted users promoted through this auto-add flow are upgraded to the `user` role automatically
- Self-hosted AgentLogs deployments can now set `PUBLIC_SHARING_ENABLED=false` to disable new public transcript sharing
</Update>
